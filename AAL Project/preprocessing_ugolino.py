...
def variance_evaluation(window):
    ...
    for field in ['roll1', 'pitch1', 'roll2', 'pitch2', 'roll3', 'pitch3','roll4', 'pitch4','total_accel_sensor_1','total_accel_sensor_2','total_accel_sensor_3','total_accel_sensor_4']:
        new_row[field] = np.var(window[field])
...
def feature_selection_discretization(dataset_sampled):
    classes = ['sittingdown','standingup','walking','standing','sitting']
    subjects = ['katia','debora','wallace','jose_carlos']
    dataset_discretized = pd.DataFrame(columns=['roll1','pitch1','roll2','pitch2','roll3','pitch3','total_accel_sensor_1','total_accel_sensor_2','total_accel_sensor_4','classes','acceleration_mean','acceleration_stdev'])
    ...
    for clas in range(5):
        ...
        for name in subjects:
            ...
            data_class_name.drop(columns=['user','gender','age','height','weight','bmi','total_accel_sensor_3','roll4','pitch4'], axis=1, inplace=True)
...
def discretize(data):
    ...
    columns=['roll1','pitch1','roll2','pitch2','roll3','pitch3','total_accel_sensor_1','total_accel_sensor_2','total_accel_sensor_4','acceleration_mean','acceleration_stdev']
